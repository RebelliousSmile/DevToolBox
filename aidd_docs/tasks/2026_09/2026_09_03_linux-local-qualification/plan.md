...
| **BLOCAGE (2026-09-03), résolu le jour même.** Le plan est passé `status: blocked` pendant la phase 2, tâche 2 étape 4, faute de contrôle « Lancer au démarrage » dans l'UI. | Aucun contrôle « Lancer au démarrage » n'existait dans l'UI (`src/ui/egui_app.rs::render_preferences_view`, section `General`) — `launch_at_startup` n'était lu qu'une fois au démarrage (`src/main.rs:133`) et n'avait aucun chemin de mutation depuis l'application. C'est un écart de fonctionnalité (feature UI absente), hors du périmètre resource-resolution-only des tâches 3. Sur instruction explicite de l'utilisateur (« reprends l'implémentation de la phase modifiée »), le correctif a été implémenté directement dans cette phase plutôt que reporté à une tâche dédiée : une case à cocher « Lancer au démarrage » a été ajoutée à `PreferencesSection::General`, persistant `config.json` puis appelant `platform::sync_startup`. Validé par un aller-retour désactivation/réactivation sur le binaire de développement (`target/debug/devtoolbox`) : le fichier `~/.config/autostart/devtoolbox.desktop` est bien supprimé puis recréé. Détail dans `phase-2.md`, tâche 2 étape 4. |
