...
Section Uninstall
  ...
  ; Preserve machine-local state and remove startup integration before any
  ; file owned by the installer is deleted. A non-zero result is a hard
  ; barrier: leaving the application installed is safer than losing data.
  ClearErrors
  ExecWait '"$INSTDIR\${MAINBINARYNAME}.exe" --prepare-uninstall' $R0
  ${If} ${Errors}
    SetErrorLevel 20
    Abort "Unable to prepare DevToolBox data for removal. Nothing was deleted."
  ${EndIf}
  ${If} $R0 != 0
    SetErrorLevel $R0
    Abort "Unable to preserve DevToolBox data (code $R0). Nothing was deleted."
  ${EndIf}
